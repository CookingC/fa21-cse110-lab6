...
function createRecipeCards() {
  ...
  for(let i=0;i<3;i++){
    var newEle = document.createElement('recipe-card');
    newEle.data = recipeData[recipes[i]];
...
function bindShowMore() {
  const buttonElement = document.getElementById('button-wrapper');
  const buttonText = document.querySelector('button');
  buttonElement.addEventListener('click', function (event) {
    var text = buttonText.textContent;
    if(text == 'Show more' ){
